  ...
  return (
    <Panel className="overflow-hidden">
      <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3 sm:px-5">
        <div>
          <h2 className="text-sm font-bold text-slate-900 sm:text-base">인기 차트</h2>
          <p className="text-xs text-slate-500">최근 3시간 조회수 TOP {popularLimit}</p>
        </div>
        <Select
          uiSize="sm"
          value={popularLimit}
            ...
            {popularDeals.map((deal, index) => (
              <li key={deal.id}>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="group grid grid-cols-[34px_1fr] gap-2 rounded-xl border border-slate-200 px-3 py-3 transition hover:border-slate-300 hover:bg-slate-50"
                >
                  <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-900 text-xs font-extrabold text-white">
                    {index + 1}
                  </div>
                  <div>
                    <p className="line-clamp-2 text-[13px] font-semibold text-slate-900 transition group-hover:text-skyline">
                      {deal.title}
                    </p>
